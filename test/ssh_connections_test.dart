...
import 'package:skysecret/desktop/ssh_connections.dart';

VaultEntry connection() => VaultEntry.create(
  title: 'Synthetic SSH', username: 'synthetic', password: 'Synthetic password!',
...
void main() {
  ...
  test('rejects shell syntax, options, URLs, invalid ports and truncated passwords', () {
    ...
    for (final host in ['', '-oProxyCommand=x', 'host & calc', 'host%PATH%', 'host!x!',
      'ssh://host', 'root@host', 'host\ncommand', 'host"', '[::1]', 'bad..host', 'host;cmd']) {
  ...
  test('encrypted round trip, conflicting endpoints and secret revocation', () async {
    ...
    final leftEntry = VaultEntry.fromJson({...entry.toJson(), 'ssh': {'host': 'left.example', 'port': 22}});
    final rightEntry = VaultEntry.fromJson({...entry.toJson(), 'ssh': {'host': 'right.example', 'port': 2222}});
  ...
  group('password request lifecycle with a mocked native channel', () {
    ...
    setUp(() async {
      ...
      service = SshConnections(channel: channel, confirmHost: (prompt) => confirm(prompt),
        onAuthorizationChanged: authorizations.add, onExit: (_) {});
    });
    tearDown(() async {
      service.dispose();
      session.lock();
    ...
    test('revocation while host confirmation is open prevents later approval and password', () async {
      ...
      confirm = (_) { opened.complete(); return confirmation.future; };
      await service.start('synthetic', session, entry);
