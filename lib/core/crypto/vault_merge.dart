...
class VaultMerge {
  ...
  static Future<VaultMerge> _combine(
    ...
    for (final id in {...baseEntries.keys, ...localEntries.keys, ...remoteEntries.keys}.toList()..sort()) {
      ...
      _alignFavorite(baseEntry, localEntry, remoteEntry);
      _alignDeletion(baseEntry, localEntry, remoteEntry);
      Map<String, dynamic>? selected;
  ...
  static void _alignFavorite(
    Map<String, dynamic>? base,
    Map<String, dynamic>? local,
    Map<String, dynamic>? remote,
  ) {
    if (base == null || local == null || remote == null) return;
    final initial = base['favorite'] == true;
    final left = local['favorite'] == true;
    final right = remote['favorite'] == true;
    final favorite = left == initial ? right : left;
    for (final entry in [base, local, remote]) {
      entry.remove('favorite');
      if (favorite) entry['favorite'] = true;
    }
  }

  static void _alignDeletion(
    Map<String, dynamic>? base,
    Map<String, dynamic>? local,
    Map<String, dynamic>? remote,
  ) {
    if (local == null || remote == null) return;
    final left = local['deletedAt'];
    final right = remote['deletedAt'];
    if (left is int && right is int && base?['deletedAt'] == null) {
      final timestamp = left > right ? left : right;
      local['deletedAt'] = timestamp;
      remote['deletedAt'] = timestamp;
    }
  }
