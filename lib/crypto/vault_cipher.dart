...
_decodeContents(List<int> plain) {
  ...
  if (json is! Map<String, dynamic> ||
      ...
      (json['entries'] as List).length >
          ([4, 5, 6, 7].contains(json['schemaVersion']) ? VaultCipher.maxItems : VaultCipher.maxEntries)) {
    throw const VaultFormatException();
  }
  ...
  for (final value in json['entries'] as List) {
    ...
    if ([4, 5, 6, 7].contains(json['schemaVersion']) && value['kind'] == 'file') {
      rawFiles++;
    } else {
      rawEntries++;
    }
  ...
  final organized = [6, 7].contains(json['schemaVersion']);
  if (!legacy &&
      (json.length != ([5, 6, 7].contains(json['schemaVersion']) ? 5 : 4) ||
          json['schemaVersion'] is! int ||
          ![2, 3, 4, 5, 6, 7].contains(json['schemaVersion']) ||
  ...
  final entries = (json['entries'] as List)
      .map(
        (value) => VaultEntry.fromJson(
          value,
          legacy: legacy,
          files: json['schemaVersion'] == 3 || [4, 5, 6, 7].contains(json['schemaVersion']),
          typed: [4, 5, 6, 7].contains(json['schemaVersion']),
          organized: organized,
          sshAllowed: json['schemaVersion'] == 7,
        ),
      )
      .toList();
  ...
  for (final entry in entries) {
    if ([4, 5, 6, 7].contains(json['schemaVersion']) || entry.attachments.isEmpty) {
  ...
  return (
    ...
    revision: [5, 6, 7].contains(json['schemaVersion']) ? VaultRevision.fromJson(json['revision']) : null,
  );
}
