...
class Translations with BaseTranslations<AppLocale, Translations> {
	...
	String vaultDeleteSubfolderQuestion({required Object name}) => 'Удалить папку «${name}»? Все её файлы и секреты останутся в разделе.';
	...
	String vaultDeleteFolderQuestion({required Object name}) => 'Удалить раздел «${name}» вместе с его папками? Все файлы и секреты останутся в корне сейфа.';
	...
	String shortcutUnavailable({required Object shortcut}) => '${shortcut} недоступно. Откройте менеджер через значок в трее.';
	...
	String get vaultCreateHelp => 'Придумайте мастер-пароль, который запомните. Он нужен для открытия данных, сбросить его нельзя.';
	...
	String get vaultMemoryProtectionFailed => 'Windows не смогла защитить память. Операция остановлена; сохранённый сейф не изменён. Закройте лишние приложения и повторите попытку.';

	String get windowPrivacyFailed => 'Windows не смогла включить защиту окна от захвата экрана.';

	String get vaultPasswordRequirements => 'Не менее 16 символов. Подходит длинная уникальная парольная фраза; заглавные буквы, цифры и спецсимволы необязательны. Очевидные пароли и повторяющиеся шаблоны не принимаются.';
	...
	String get vaultLockWhenHiddenHelp => 'Esc, крестик, сворачивание и уход в трей при потере фокуса блокируют сейф и закрывают редакторы. Несохранённые изменения теряются. Можно отключить отдельно от таймера.';

	String get vaultSnapshotCleanupWarning => 'Текущий сейф использует новый пароль, но часть локальных снимков не удалось удалить: они могут открываться старым паролем. Закройте программы, использующие эти файлы, и откройте сейф снова для повторной очистки. Внешние экспорты и история GitHub остаются отдельно.';

	String get vaultPasswordBackupWarning => 'После сохранения нового пароля локальная история восстановления и кэш синхронизации удаляются. Прежние экспорты и версии в GitHub по-прежнему открываются старым паролем. Если он скомпрометирован, проверьте новую независимую копию и замените репозиторий резервных копий; старые репозитории и скачанные копии автоматически не отзываются. Смените также раскрытые пароли учётных записей. Новые сейфы и смена пароля используют формат v2; обновите все устройства до SkySecret 0.2.0 или новее.';
	...
	String get vaultAttachmentHelp => 'До 20 МиБ на файл, до 50 МиБ всего. Изменения применяются кнопкой «Сохранить». Извлечённые файлы не зашифрованы.';

	String get vaultImportHelp => 'Введите пароль резервной копии. Она будет добавлена как отдельный сейф; существующие данные сохранятся.';

	String get vaultImportDone => 'Сейф проверен и импортирован.';

	String get vaultCurrentPassword => 'Текущий мастер-пароль';

	String get vaultDestinationExists => 'Файл уже существует или сейф изменился. Выберите новое имя файла; при смене пароля откройте сейф заново.';
	...
	String vaultDeleteVaultQuestion({required Object name}) => 'Удалить сейф «${name}» с этого компьютера? Все его записи и файлы будут удалены без возможности отмены. Другие сейфы и экспортированные резервные копии сохранятся.';

	String get vaultDeleted => 'Сейф удалён с этого компьютера.';

	String get vaultAttachmentLimit => 'Лимит: 20 МиБ на файл, 50 МиБ всего.';

	String get vaultChangePasswordHelp => 'Задайте новый пароль. Старые резервные копии останутся доступны по прежнему паролю.';

	String get fileEditorSaveFailed => 'Не удалось сохранить. Сейф занят, файл изменён или превышен лимит 2 МиБ. Повторите попытку; при конфликте откройте файл заново.';

	String get vaultDeleteFile => 'Удалить файл';

	String get fileEditorShortcut => 'Ctrl+S · сохранить в сейф';

	String get fileEditorUnsupported => 'Просмотр недоступен для этого формата, кодировки или файла больше 2 МиБ. Сохраните исходный файл на компьютер.';
	...
	String get vaultDropUnavailable => 'Перетаскивание файлов недоступно. Перезапустите приложение или используйте «Добавить файл».';
	...
	String get githubNetworkError => 'GitHub недоступен. Данные сохранены локально; отправка будет повторена после восстановления сети.';
	...
	String get githubMissingError => 'Подключённый репозиторий недоступен. Проверьте доступ в GitHub; новая замена не создаётся.';

	String get githubConflictHelp => 'Удалённая версия изменилась. Автоматическая отправка остановлена. Можно сохранить локальные сейфы как новые копии, сохранив обе версии, или восстановить удалённую копию отдельным сейфом.';

	String get githubRepositoryChangedError => 'Выбран другой репозиторий: у приложения сохранена привязка к прежним копиям. Укажите прежний репозиторий. Если вы удалили его и создали заново с тем же именем, GitHub считает его новым. Локальные сейфы не изменены.';

	String get githubRepositorySetupError => 'В новом репозитории найдены файлы помимо README.md. Для первого подключения создайте отдельный приватный репозиторий только с README.md. Если здесь уже есть копии SkySecret, снимите флажок нового репозитория. Ничего не удаляйте из существующих копий.';

	String get githubFormatError => 'Структура резервной копии не поддерживается или повреждена. Рабочие сейфы не изменены.';

	String get githubStorageError => 'Не удалось прочитать или сохранить защищённые настройки GitHub. Отправка остановлена; проверьте доступ к локальному хранилищу.';

	String get githubIdentityError => 'Нужен токен прежнего владельца. Автоматический перенос сейфов в другой аккаунт запрещён.';

	String get githubConfigurationError => 'Укажите fine-grained token (github_pat_…) и полный адрес https://github.com/владелец/репозиторий или владелец/репозиторий. Одного https://github.com недостаточно.';
	...
	String get githubRestoreHelp => 'Нужен мастер-пароль выбранной копии. Для работы на нескольких устройствах оставьте привязку включённой. Для независимой копии выключите её. Существующие сейфы не заменяются. Названия зашифрованы, поэтому до открытия видны идентификаторы.';
	...
	String get githubHistoryHelp => 'История GitHub хранит старые зашифрованные версии: смена мастер-пароля их не отзывает. Отключение удаляет токен с этого устройства; для отзыва удалите его в GitHub → Settings → Developer settings → Personal access tokens.';
	...
	String get githubSetupHelp => 'GitHub хранит зашифрованные копии ваших сейфов. Настройте подключение один раз по шагам ниже. Регистрация OAuth App или GitHub App не нужна.';

	String get githubSetupRepositoryStep => '1. Место для копий';

	String get githubSetupRepositoryHelp => 'На GitHub выберите свой личный аккаунт, задайте имя репозитория, выберите Private и включите Add a README file. Не добавляйте .gitignore или лицензию. Если репозиторий с копиями уже есть, используйте его.';

	String get githubSetupTokenStep => '2. Ключ доступа';

	String get githubSetupTokenHelp => 'Кнопка ниже открывает создание fine-grained token. Resource owner — ваш аккаунт. Repository access → Only select repositories → только репозиторий копий. Contents → Read and write; Metadata → Read-only. Остальные права не добавляйте. Выберите срок действия, нажмите Generate token и скопируйте github_pat_… в поле ниже.';

	String get githubSetupGuide => 'Открыть подробную инструкцию';

	String get githubSetupConnectStep => '3. Подключение';

	String get githubSetupConnectHelp => 'Вставьте полный адрес репозитория и токен. Подтвердите выбранные права; для нового репозитория с README отметьте подготовку. Нажмите «Подключить» — сохранённые сейфы будут отправлены автоматически. Дождитесь завершения без ошибки.';
	...
	String get githubTokenConfirmation => 'В настройках токена выбран только этот репозиторий; из прав добавлено только Contents → Read and write.';
	...
	String get syncDone => 'Открытый сейф синхронизирован с проверенной версией GitHub. На другом устройстве нажмите «Синхронизировать», чтобы получить изменения.';

	String syncConflicts({required Object count}) => 'Сейф синхронизирован. Конфликтов: ${count}. Варианты записей помечены и сохранены. Откройте меню сейфа → Разобрать конфликты, затем синхронизируйте результат.';

	String get syncFinishEditing => 'Сначала сохраните или отмените правки и закройте редакторы файлов. Затем синхронизируйте сейф.';

	String get syncUnlock => 'Откройте нужный сейф мастер-паролем. На новом устройстве выберите копию GitHub и включите привязку для синхронизации.';

	String get syncKeyChanged => 'Копия использует другой ключ. Автоматическое объединение отключено: локальные записи не будут зашифрованы ключом этой копии. Текущий сейф сохранён. Можно восстановить удалённую версию отдельным сейфом и проверить её. При раскрытии прежнего пароля подключите локальный сейф к новому репозиторию.';

	String get syncLink => 'Привязать к этой копии для ручной синхронизации между устройствами';

	String get syncAlreadyLinked => 'Эта копия уже подключена на этом компьютере. Откройте выбранный сейф и нажмите «Синхронизировать».';

	String get syncVaultHelp => 'Кнопка «Синхронизировать» получает и отправляет изменения открытого сейфа. После первой синхронизации этот сейф обменивается данными только по кнопке. Фонового опроса нет.';

	String get syncRollback => 'Обнаружена старая версия или несовместимая история. Синхронизация остановлена, локальные данные сохранены. Проверьте другие устройства и историю сейфа.';

	String get vaultHistory => 'История и восстановление';

	String get vaultHistoryHelp => 'Выберите зашифрованный снимок. Восстановление создаёт отдельный сейф и требует пароль того времени. Храним до 20 предыдущих состояний на сейф в пределах 256 МиБ.';

	String get vaultHistoryEmpty => 'Предыдущих состояний пока нет.';

	String get syncReviewConflicts => 'Разобрать конфликты';

	String get syncReviewHelp => 'Раскройте варианты для сравнения. Выбор оставит один вариант, остальные останутся в локальной истории. Файлы можно открыть из списка сейфа.';
	...
	String get syncKeepVariantQuestion => 'Оставить выбранный вариант? Другие варианты этой группы будут удалены из текущего сейфа. Предыдущее состояние останется в локальной истории.';

	String get syncRelink => 'Связать с открытым сейфом';

	String get syncRelinkHelp => 'Восстановить связь открытого сейфа с этой копией? Приложение проверит принадлежность и историю. При несовместимой истории связь не будет изменена. Данные отправятся только после нажатия «Синхронизировать».';
	...
	String get githubRecoverConnectionHelp => 'Журнал подключения повреждён. Сбросить подключение и ввести токен заново? Локальные сейфы и история останутся. Все существующие сейфы перейдут в ручной режим; связь с копиями нужно восстановить кнопкой цепочки рядом с копией.';
	...
	String get sshHelp => 'Пароль передаётся один раз, по запросу OpenSSH. Доступ к паролю прекращается при блокировке сейфа или через 2 минуты после запуска. Во время входа менеджер не скрывается автоматически. Открытый терминал продолжает работать после блокировки.';

	String get sshHostKeyTitle => 'Ключ SSH-сервера';

	String get sshHostKeyHelp => 'Сверьте отпечаток ключа с владельцем сервера по независимому каналу. Подтверждайте только знакомый сервер. OpenSSH сохранит ключ в known_hosts.';
	...
	String get sshFailed => 'SSH завершился с ошибкой или запуск не удался. Проверьте адрес, доступность сервера, пароль и ключ сервера.';
	...
	String get sshInvalid => 'Укажите DNS-имя или IP без команды, порт 1–65535 и пользователя латиницей (буквы, цифры, _, ., -, допустим символ доллара в конце). Пароль обязателен: до 1000 байт UTF-8, без переноса строк и NUL.';

	String get captureVisible => 'Показывать при записи экрана';

	String get captureVisibleHelp => 'Включите, чтобы менеджер и текстовые редакторы были видны на скриншотах, в записи и демонстрации экрана. Содержимое окон может попасть в запись. По умолчанию выключено.';

	String get captureSettingFailed => 'Не удалось применить настройку захвата ко всем окнам. Проверьте их видимость в программе записи.';
...
extension on Translations {
	dynamic _flatMapFunction(String path) {
		return switch (path) {
			...
			'vaultDeleteSubfolderQuestion' => ({required Object name}) => 'Удалить папку «${name}»? Все её файлы и секреты останутся в разделе.',
			...
			'vaultDeleteFolderQuestion' => ({required Object name}) => 'Удалить раздел «${name}» вместе с его папками? Все файлы и секреты останутся в корне сейфа.',
			...
			'shortcutUnavailable' => ({required Object shortcut}) => '${shortcut} недоступно. Откройте менеджер через значок в трее.',
			...
			'vaultCreateHelp' => 'Придумайте мастер-пароль, который запомните. Он нужен для открытия данных, сбросить его нельзя.',
			...
			'vaultImportHelp' => 'Введите пароль резервной копии. Она будет добавлена как отдельный сейф; существующие данные сохранятся.',
			'vaultImportDone' => 'Сейф проверен и импортирован.',
			'vaultCurrentPassword' => 'Текущий мастер-пароль',
			'vaultDestinationExists' => 'Файл уже существует или сейф изменился. Выберите новое имя файла; при смене пароля откройте сейф заново.',
			...
			'vaultDeleteVaultQuestion' => ({required Object name}) => 'Удалить сейф «${name}» с этого компьютера? Все его записи и файлы будут удалены без возможности отмены. Другие сейфы и экспортированные резервные копии сохранятся.',
			'vaultDeleted' => 'Сейф удалён с этого компьютера.',
			'vaultAttachmentLimit' => 'Лимит: 20 МиБ на файл, 50 МиБ всего.',
			'vaultChangePasswordHelp' => 'Задайте новый пароль. Старые резервные копии останутся доступны по прежнему паролю.',
			...
			'vaultDropUnavailable' => 'Перетаскивание файлов недоступно. Перезапустите приложение или используйте «Добавить файл».',
			...
			'githubNetworkError' => 'GitHub недоступен. Данные сохранены локально; отправка будет повторена после восстановления сети.',
			...
			'githubMissingError' => 'Подключённый репозиторий недоступен. Проверьте доступ в GitHub; новая замена не создаётся.',
			...
			'githubIdentityError' => 'Нужен токен прежнего владельца. Автоматический перенос сейфов в другой аккаунт запрещён.',
			...
			'githubTokenConfirmation' => 'В настройках токена выбран только этот репозиторий; из прав добавлено только Contents → Read and write.',
			...
			'syncConflicts' => ({required Object count}) => 'Сейф синхронизирован. Конфликтов: ${count}. Варианты записей помечены и сохранены. Откройте меню сейфа → Разобрать конфликты, затем синхронизируйте результат.',
			'syncFinishEditing' => 'Сначала сохраните или отмените правки и закройте редакторы файлов. Затем синхронизируйте сейф.',
			...
			'syncAlreadyLinked' => 'Эта копия уже подключена на этом компьютере. Откройте выбранный сейф и нажмите «Синхронизировать».',
			...
			'sshFailed' => 'SSH завершился с ошибкой или запуск не удался. Проверьте адрес, доступность сервера, пароль и ключ сервера.',
			...
			'captureSettingFailed' => 'Не удалось применить настройку захвата ко всем окнам. Проверьте их видимость в программе записи.',
