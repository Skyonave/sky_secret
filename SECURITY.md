...
Text input Copy/Cut uses the protected clipboard path; revealed conflict passwords use the explicit Copy button. IME personalized learning is disabled for text inputs. Completed copies expire 10 seconds after copying; hiding locks the vault without clearing these copies early or extending their deadline. Pending copies are revoked on hiding. Manual, idle and Windows locking still clear the app's current clipboard copy immediately. Cleanup preserves newer copies from other applications. Clipboard expiration is best-effort: a busy clipboard delays cleanup until a retry, and abnormal process or OS termination can prevent clearing.
...
Copy/Cut в текстовых полях проходят через защищённый буфер обмена; раскрытый пароль конфликта копируется отдельной кнопкой. Персонализированное обучение IME отключено для текстовых полей. Завершённые копии очищаются через 10 секунд после копирования; скрытие блокирует сейф, сохраняя эти копии до исходного срока. Незавершённое копирование при скрытии отзывается. Ручная блокировка, блокировка по бездействию и блокировка Windows сразу очищают текущую копию приложения. Более новые копии других приложений сохраняются. Очистка выполняется по возможности: занятый буфер задерживает её до повтора, а аварийное завершение приложения или ОС может помешать удалению.
